...
const MovieDetail = () => {
  ...
  const imageAPI = `https://image.tmdb.org/t/p/w1280${moviesDetail?.poster_path}`;
  const defaultImage =
  ...
  return (
    <>
      <div className="container px-10 mx-auto py-5 h-screen">
        <h1 className="text-center dark:text-white text-3xl">
          {moviesDetail?.title}
        </h1>
        <h1>
        ...
        <div className="container flex justify-center px-10">
          <div className="flex flex-col lg:flex-row max-w-6xl rounded-lg shadow-lg    bg-white dark:bg-gray-900">
            <img
              ...
              src={moviesDetail?.poster_path ? imageAPI : defaultImage}
              alt="poster"
            />
            <div className="p-6 flex flex-col justify-between">
              ...
              <ul className="bg-gray-100 rounded-lg border border-gray-400 text-gray-900">
                ...
                <li className="px-6 py-2 border-b border-gray-400 w-full">
                  <b> Genre : </b>
                  {moviesDetail?.genres?.map((item, index) =>
                    moviesDetail?.genres?.length - 1 === index ? (
                      <span>{item.name} </span>
                    ) : (
                      <span>{item.name}, </span>
                    )
                ...
                <li className="px-6 py-2 border-b border-gray-400 w-full">
                  <b> Language : </b>
                  {moviesDetail?.spoken_languages?.map((item, index) =>
                    moviesDetail?.spoken_languages?.length - 1 === index ? (
                      <span>{item.name} </span>
                    ) : (
                      <span>{item.name}, </span>
                    )
                ...
                <li className="px-6 py-2 border-gray-400 w-full rounded-t-lg">
                  ...
                  <button
                    ...
                    onClick={() =>
                      window.location.replace(moviesDetail?.homepage)
                    }>
                    <svg
                      aria-hidden="true"
                      className="w-5 h-5 mr-2"
